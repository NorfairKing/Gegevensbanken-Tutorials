...
\begin{itemize}
\item $M$: het huidig aantal emmers.
\item $d$: de diepte van een emmer.
\item $m_i$: de $i$-de emmer
\item $h_0$: de initi\"ele hashfunctie.
\item $h_j$: de $j$-de hashfunctie.
...
l = \frac{r}{dM}
\]
\item $j$: het aantal verdubbelingen in grootte dat de structuur al heeft ondergaan.
\end{itemize}

\subsection{De hashfunctie}
Definieer de initiele hash functie $h_0$ eenvoudigweg als volgt. 
\[
h_0(K) = K\ mod\ M
\]
De bits van $K$ worden dus als getal beschouwd om mee te rekenen.
Definieer bovendien de hash functie $h_j$ als volgt.
\[
h_{j}(K) = K\ mod\ (2^{j}M)
\]
...
Begin met een lijst van $M$ buckets en geef elke bucket een diepte $d$. $n$ begint bij $0$ alsook $j$. Kies $L$ als bovengrens voor de laadfactor.\footnote{Voor $L$ wordt vaak $0.7$ gekozen.}
\subsubsection{toevoeging}
Gegeven een record $r$ met sleutel $k$, voegen we deze in als volgt.
\begin{enumerate}
\item Hash de sleutel met hashfunctie $h_j$ om $h$ te bekomen. $h$ is dus een emmernummer.
\[
h = h_{j}(k)
\]
\item Voeg de record $r$ toe aan de $m$-de emmer $m_h$. (Als $m_h$ vol zit, gebruik dan de overloop.)
...
\item Maak een nieuwe emmer op het einde van de lijst ($m_{n+M}$)
\item Splits emmer $m_n$ door alle records in $m_n$ opnieuw te hashen, maar deze keer met hashfunctie $h_{j+1}$. Alle record die in $m_n$ zaten komen nu ofwel terug in $m_n$ terecht ofwel in de nieuwe emmer $m_{n+M}$
\item Hoog $n$ op. $M$ is nu ook $1$ groter geworden.
\item Als $n \ge \frac{M}{2}$, hoog dan $j$ op, en zet $n$ terug op $0$. Dit gebeurt wanneer de lijst dubbel zo lang is geworden als de originele lijst.
